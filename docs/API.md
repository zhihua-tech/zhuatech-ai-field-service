...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `workOrderNo` | String | 现场工单号 |
| `priority` | String | 优先级(P1-P4) |
| `travelMinutes` | int | 预计路程分钟 |
| `slaRemainingMinutes` | int | SLA剩余分钟 |
| `technicianSkillMatch` | double | 技能匹配率(%) |
| `partsAvailabilityRate` | double | 备件齐套率(%) |
| `firstTimeFixProbability` | double | 一次修复概率(%) |
